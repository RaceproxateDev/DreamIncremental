...
function BuyUpgWithProb(id, curr) {
    if (Data[curr].gte(Data.Buyables[id].price) && !Data.Buyables[id].amount.gte(Data.Buyables[id].max) && Data.Buyables[id].probability.eq(true)) {
        Data[curr] = Data[curr].sub(Data.Buyables[id].price)
        Data.Buyables[id].amount = Data.Buyables[id].amount.add(1)
        Data.Buyables[id].price = Data.Buyables[id].price.times(Data.Buyables[id].scale)
        Data.Buyables[id].probability = Data.Buyables[id].probability.sub(Data.Buyables[id].probDeScale)
    } else if (Data[curr].gte(Data.Buyables[id].price) && !Data.Buyables[id].amount.gte(Data.Buyables[id].max) && !Data.Buyables[id].probability.eq(true)) {
        Data[curr] = Data[curr].sub(Data.Buyables[id].price)
    }
}
...
function buyMaxUpgWithProb(id, curr) {
    while (Data[curr].gte(Data.Buyables[id].price) && !Data.Buyables[id].amount.gte(Data.Buyables[id].max) && !Data[curr].lt(Data.Buyables[id].price) && Data.Buyables[id].probability.eq(true)) {
        Data[curr] = Data[curr].sub(Data.Buyables[id].price)
        Data.Buyables[id].amount = Data.Buyables[id].amount.add(1)
        Data.Buyables[id].price = Data.Buyables[id].price.times(Data.Buyables[id].scale)
        Data.Buyables[id].probability = Data.Buyables[id].probability.sub(Data.Buyables[id].probDeScale)
...
function resetBuyablesWithProb(limit, start) {
    for (let i = start; i <= limit; i++) {
        Data.Buyables[i] = {
            amount: new OmegaNum(Template.Buyables[i].amount),
            max: new OmegaNum(Template.Buyables[i].max),
